...
function SignIn({onSwitch,onSuccess}){
  ...
  const submit=()=>{if(!email||!pwd){setErr('Please fill in all fields.');return;}setErr('');setLoading(true);setTimeout(()=>{setLoading(false);onSuccess();},1400);};
  return(
    <div className="page">
      <div className="auth-card">
        ...
        <div className="social-row">
          <button className="social-btn"><span style={{fontWeight:800,color:'#4285F4'}}>G</span> Google</button>
          <button className="social-btn"><span style={{fontWeight:800}}>⎔</span> GitHub</button>
        </div>
  ...
  return(
    <div className="page">
      <div className="auth-card">
        ...
        <button className="btn-primary" onClick={()=>{setLoading(true);setTimeout(()=>{setLoading(false);onSuccess();},1500);}} disabled={loading||!agreed}>{loading?'Creating account…':'Create Account →'}</button>
      </div>
    </div>
  );
}
...
// ── BUILDER ───────────────────────────────────────────────────────────────────
const TEMPLATES=[
  {id:'launch',name:'Launch',colors:['#6366f1','#4f46e5','#eef0ff']},
  {id:'studio',name:'Studio',colors:['#0f172a','#334155','#f1f5f9']},
  {id:'neon',  name:'Neon',  colors:['#06b6d4','#0e7490','#ecfeff']},
];
const PALETTES=[
  {id:'indigo',from:'#6366f1',to:'#a78bfa'},
  {id:'cyan',  from:'#06b6d4',to:'#0ea5e9'},
  {id:'rose',  from:'#f43f5e',to:'#fb923c'},
  {id:'emerald',from:'#10b981',to:'#14b8a6'},
  {id:'amber', from:'#f59e0b',to:'#ef4444'},
  {id:'violet',from:'#7c3aed',to:'#db2777'},
];
const PLANS=[
  {id:'free', name:'Free',  price:'Rs. 0',     period:'',    features:['1 site','5 pages','Community support','yourname.site.io']},
  {id:'pro',  name:'Pro',   price:'Rs. 1,990', period:'/mo', features:['Unlimited sites','Custom domain','Analytics','Priority support'],popular:true},
  {id:'team', name:'Team',  price:'Rs. 4,990', period:'/mo', features:['5 seats','Shared workspace','SSO','SLA uptime']},
];

function Builder({onNext}){
  ...
  const taken=['mysite','demo','test','admin'];
  const available=subdomain.length>2&&!taken.includes(subdomain.toLowerCase());
  const pal=PALETTES.find(p=>p.id===palette);
  const pg=`linear-gradient(135deg,${pal.from},${pal.to})`;
  const labels=['Name your site','Pick a template','Choose your plan'];
  return(
          ...
          {labels.map((label,i)=>(
            <div key={i} style={{display:'flex',alignItems:'center'}}>
              <div className="step-item">
                <div className={`step-dot ${step>i+1?'done':step===i+1?'active':''}`}>{step>i+1?'✓':i+1}</div>
                <span className="step-label">{label}</span>
              </div>
              {i<labels.length-1&&<div className={`step-line ${step>i+1?'done':''}`} style={{margin:'0 4px 22px'}}/>}
            </div>
          ))}
        </div>
        <h1 className="builder-title">
          {step===1&&<>Build your site,<br/><span className="gradient-text">before you pay</span></>}
          {step===2&&<>Choose your<br/><span className="gradient-text">design template</span></>}
          {step===3&&<>Pick a plan<br/><span className="gradient-text">&amp; go live</span></>}
        </h1>
        <p className="builder-subtitle">
          {step===1&&'Set up your subdomain and start customizing. No credit card needed to start.'}
          {step===2&&'Pick a starting point. You can change everything later.'}
          {step===3&&'Publish free or unlock more power. All prices in Sri Lankan Rupees (Rs.).'}
        </p>
      </div>
      <div className="builder-main">
        {step===1&&<>
          <div className="builder-card">
            <div className="bc-label">Your subdomain</div><div className="bc-title">Claim your address</div><div className="bc-desc">Your site URL. Connect a custom domain later.</div>
            <div className="subdomain-input-wrap">
              <span className="subdomain-prefix">https://</span>
              <input className="subdomain-input" placeholder="yourname" value={subdomain} onChange={e=>setSubdomain(e.target.value.replace(/[^a-z0-9-]/gi,'').toLowerCase())}/>
              <span className="subdomain-suffix">.launchpad.io</span>
            </div>
            {subdomain.length>2&&<div className={`availability ${available?'avail':'taken'}`}>{available?'✓':'✕'} {subdomain}.launchpad.io is {available?'available!':'taken'}</div>}
          </div>
          <div className="builder-card">
            <div className="bc-label">Site name</div><div className="bc-title">What's your site called?</div><div className="bc-desc">Appears in the browser tab and as your headline.</div>
            <div className="input-wrap"><span className="input-icon">✦</span><input type="text" placeholder="e.g. My Awesome Product" value={siteName} onChange={e=>setSiteName(e.target.value)}/></div>
          </div>
          <div className="builder-card builder-card-full">
            <div className="bc-label">Live preview</div>
            <div className="preview-pane">
              <div className="preview-toolbar"><div className="tb-dot" style={{background:'#f43f5e'}}/><div className="tb-dot" style={{background:'#f59e0b'}}/><div className="tb-dot" style={{background:'#10b981'}}/><div className="tb-url">{subdomain||'yourname'}.launchpad.io</div></div>
              <div className="preview-body" style={{background:`linear-gradient(135deg,${pal.from}0d,${pal.to}06)`}}>
                <div className="prev-headline" style={{background:pg,opacity:0.7}}/>
                <div className="prev-sub" style={{background:'rgba(0,0,0,0.08)'}}/>
                <div className="prev-cta" style={{background:pg}}/>
                <div className="prev-img" style={{background:`linear-gradient(135deg,${pal.from}14,${pal.to}0a)`,border:`1px solid ${pal.from}33`}}/>
              </div>
            </div>
          </div>
          <div className="builder-cta">
            <div className="cta-text"><h3>Ready to design your site?</h3><p>Pick a template next and watch it come to life.</p></div>
            <button className="btn-launch" onClick={()=>setStep(2)} disabled={!subdomain||subdomain.length<3}>Next: Pick Template →</button>
          </div>
        </>}
        {step===2&&<>
          <div className="builder-card">
            <div className="bc-label">Template</div><div className="bc-title">Starting layout</div><div className="bc-desc">Pick a structure. Everything is customizable.</div>
            <div className="template-grid">
              {TEMPLATES.map(t=>(
                <div key={t.id} className={`template-item ${template===t.id?'selected':''}`} onClick={()=>setTemplate(t.id)}>
                  <div className="template-preview" style={{background:t.colors[2]}}>
                    <div className="tprev-bar" style={{background:t.colors[0]}}/>
                    <div className="tprev-hero" style={{background:`linear-gradient(135deg,${t.colors[0]}44,${t.colors[1]}22)`}}><div style={{width:26,height:3,borderRadius:2,background:t.colors[0]}}/></div>
                    <div className="tprev-row">{[0.5,0.35,0.2].map((o,i)=><div key={i} className="tprev-col" style={{background:`${t.colors[0]}${Math.round(o*255).toString(16).padStart(2,'0')}`}}/>)}</div>
                  </div>
          ...
          <div className="builder-card">
            <div className="bc-label">Color palette</div><div className="bc-title">Brand colors</div><div className="bc-desc">Choose a color scheme matching your brand.</div>
            <div className="color-swatches">{PALETTES.map(p=><div key={p.id} className={`swatch ${palette===p.id?'selected':''}`} style={{background:`linear-gradient(135deg,${p.from},${p.to})`}} onClick={()=>setPalette(p.id)}/>)}</div>
          </div>
          <div className="builder-card builder-card-full">
            <div className="bc-label">Preview with your selections</div>
            <div className="preview-pane">
              <div className="preview-toolbar"><div className="tb-dot" style={{background:'#f43f5e'}}/><div className="tb-dot" style={{background:'#f59e0b'}}/><div className="tb-dot" style={{background:'#10b981'}}/><div className="tb-url">{subdomain||'yourname'}.launchpad.io</div></div>
              <div className="preview-body" style={{background:`linear-gradient(135deg,${pal.from}0d,${pal.to}06)`}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
                  <div style={{height:5,width:50,borderRadius:3,background:pg}}/><div style={{display:'flex',gap:4}}>{[1,2,3].map(i=><div key={i} style={{height:5,width:16,borderRadius:2,background:'rgba(0,0,0,0.07)'}}/>)}</div>
                </div>
                <div className="prev-headline" style={{background:pg,opacity:0.75,width:'65%'}}/><div className="prev-sub" style={{background:'rgba(0,0,0,0.07)',width:'45%'}}/><div className="prev-cta" style={{background:pg}}/>
                <div style={{display:'flex',gap:8,flex:1}}>{[1,2,3].map(i=><div key={i} style={{flex:1,minHeight:44,borderRadius:8,background:`linear-gradient(135deg,${pal.from}14,${pal.to}0a)`,border:`1px solid ${pal.from}33`}}/>)}</div>
              </div>
            </div>
          </div>
          <div className="builder-cta">
            <div className="cta-text"><h3>Loving the look?</h3><p>Choose a plan and your site goes live instantly.</p></div>
            <div style={{display:'flex',gap:10}}>
              <button className="btn-launch" style={{background:'var(--surface2)',color:'var(--text)',border:'1px solid var(--border)',boxShadow:'none'}} onClick={()=>setStep(1)}>← Back</button>
              <button className="btn-launch" onClick={()=>setStep(3)}>Next: Choose Plan →</button>
            </div>
          </div>
        </>}
        {step===3&&<>
          <div className="builder-card builder-card-full">
            <div className="bc-label">Pricing · All prices in Sri Lankan Rupees (Rs.)</div><div className="bc-title">Choose your plan</div><div className="bc-desc" style={{marginBottom:22}}>Start free forever. Upgrade anytime to unlock more features.</div>
            <div className="plan-cards">{PLANS.map(p=><div key={p.id} className={`plan-card ${plan===p.id?'selected':''} ${p.popular?'popular':''}`} onClick={()=>setPlan(p.id)}><div className="plan-name">{p.name}</div><div className="plan-price">{p.price}<span>{p.period}</span></div><ul className="plan-features">{p.features.map(f=><li key={f}>{f}</li>)}</ul></div>)}</div>
          </div>
          <div className="builder-card builder-card-full" style={{background:'rgba(99,102,241,0.03)'}}>
            <div className="bc-label">Your site summary</div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>
              {[['URL',`${subdomain}.launchpad.io`],['Template',TEMPLATES.find(t=>t.id===template)?.name||'—'],['Plan',PLANS.find(p=>p.id===plan)?.name||'—']].map(([k,v])=>(
                <div key={k} style={{padding:'12px 14px',background:'white',borderRadius:10,border:'1px solid var(--border)',boxShadow:'var(--shadow-sm)'}}>
                  <div style={{fontSize:10,color:'var(--text-dim)',marginBottom:3,textTransform:'uppercase',letterSpacing:1,fontWeight:700}}>{k}</div>
                  <div style={{fontSize:13,fontWeight:700,fontFamily:'Syne,sans-serif',color:'var(--text)'}}>{v||'—'}</div>
                </div>
              ))}
            </div>
          </div>
          <div className="builder-cta">
            <div className="cta-text"><h3>Ready to launch? 🚀</h3><p>{plan==='free'?'Go live for free — no card required.':`Checkout for the ${PLANS.find(p=>p.id===plan)?.name} plan.`}</p></div>
            <div style={{display:'flex',gap:10}}>
              <button className="btn-launch" style={{background:'var(--surface2)',color:'var(--text)',border:'1px solid var(--border)',boxShadow:'none'}} onClick={()=>setStep(2)}>← Back</button>
              <button className="btn-launch" onClick={()=>onNext(plan,subdomain)}>{plan==='free'?'Launch Site Free →':'Continue to Payment →'}</button>
            </div>
          </div>
        </>}
      </div>
...
// ── PAYMENT ───────────────────────────────────────────────────────────────────
const PLAN_INFO={free:{name:'Free Plan',price:0},pro:{name:'Pro Plan',price:1990},team:{name:'Team Plan',price:4990}};

function Payment({plan,subdomain,onBack}){
  const [method,setMethod]=useState('card');
  const [wallet,setWallet]=useState('ezcash');
  const [card,setCard]=useState({number:'',expiry:'',cvv:'',name:''});
  const [billing,setBilling]=useState('monthly');
  const [copied,setCopied]=useState('');
  const [paid,setPaid]=useState(false);
  const [loading,setLoading]=useState(false);
  const pd=PLAN_INFO[plan]||PLAN_INFO.pro;
  const price=billing==='annual'?Math.round(pd.price*0.83):pd.price;
  const discount=billing==='annual'?Math.round(pd.price*2):0;
  const tax=Math.round(price*0.08);
  const total=price+tax;
  const fmt=n=>`Rs. ${n.toLocaleString('en-LK')}`;
  const copyText=(text,key)=>{navigator.clipboard?.writeText(text).catch(()=>{});setCopied(key);setTimeout(()=>setCopied(''),2000);};
  const fmtCard=v=>v.replace(/\D/g,'').replace(/(.{4})/g,'$1 ').trim().slice(0,19);
  const fmtExp=v=>v.replace(/\D/g,'').replace(/^(\d{2})(\d)/,'$1/$2').slice(0,5);
  const WALLETS=[{id:'ezcash',emoji:'📲',name:'eZ Cash',desc:'Dialog eZ Cash'},{id:'mcash',emoji:'💵',name:'mCash',desc:'Mobitel mCash'},{id:'frimi',emoji:'🟣',name:'FriMi',desc:'NTB FriMi'},{id:'payhere',emoji:'💙',name:'PayHere',desc:'PayHere Wallet'}];
  const WMAPS={ezcash:'eZ Cash',mcash:'mCash',frimi:'FriMi',payhere:'PayHere'};
  return(
    <div className="payment-page">
      <div className="payment-container">
        <div className="payment-header">
          <button onClick={onBack} style={{border:'none',background:'none',cursor:'pointer',color:'var(--primary)',fontWeight:600,fontSize:14,marginBottom:10,display:'flex',alignItems:'center',gap:5,padding:0}}>← Back to builder</button>
          <h2>Complete your payment</h2>
          <p>Secure checkout · All prices in Sri Lankan Rupees (Rs.)</p>
        </div>
        <div className="payment-grid">
          ...
          <div>
            ...
            <div className="pay-card">
              ...
              <div className="pay-methods">
                {[{id:'card',icon:'💳',label:'Card'},{id:'bank',icon:'🏦',label:'Bank Transfer'},{id:'wallet',icon:'📱',label:'eZ Cash / Wallet'}].map(m=>(
                  <button key={m.id} className={`pay-method-btn ${method===m.id?'active':''}`} onClick={()=>setMethod(m.id)}>
                    <span className="pay-method-icon">{m.icon}</span>{m.label}
                  </button>
                ))}
              </div>
              ...
              {method==='card'&&(
                <div>
                  <div className="card-input-group"><label>Cardholder Name</label><div className="input-wrap"><span className="input-icon">👤</span><input type="text" placeholder="Name on card" value={card.name} onChange={e=>setCard(c=>({...c,name:e.target.value}))}/></div></div>
                  <div className="card-input-group">
                    <label>Card Number</label>
                    <div className="input-wrap card-number-wrap">
                      <span className="input-icon">💳</span>
                      <input type="text" placeholder="0000 0000 0000 0000" value={card.number} style={{paddingLeft:38,paddingRight:72,letterSpacing:'1.5px'}} onChange={e=>setCard(c=>({...c,number:fmtCard(e.target.value)}))}/>
                      <div className="card-chip"><div className="chip-card" style={{background:'linear-gradient(135deg,#1a1f71,#009ee0)'}}/><div className="chip-card" style={{background:'linear-gradient(135deg,#eb001b,#f79e1b)'}}/></div>
                    </div>
                  </div>
                  <div className="card-input-row">
                    <div className="card-input-group" style={{marginBottom:0}}><label>Expiry Date</label><div className="input-wrap"><span className="input-icon" style={{fontSize:12}}>📅</span><input type="text" placeholder="MM / YY" value={card.expiry} onChange={e=>setCard(c=>({...c,expiry:fmtExp(e.target.value)}))}/></div></div>
                    <div className="card-input-group" style={{marginBottom:0}}><label>CVV / CVC</label><div className="input-wrap"><span className="input-icon" style={{fontSize:12}}>🔐</span><input type="password" placeholder="•••" maxLength={4} value={card.cvv} onChange={e=>setCard(c=>({...c,cvv:e.target.value.replace(/\D/,'').slice(0,4)}))}/></div></div>
                  </div>
                  <div style={{marginTop:14,padding:'10px 13px',background:'rgba(16,185,129,0.06)',border:'1px solid rgba(16,185,129,0.2)',borderRadius:8,fontSize:12,color:'#059669',display:'flex',alignItems:'center',gap:6}}>🔒 Your card details are encrypted and secure. We never store your CVV.</div>
                </div>
              )}

              {method==='bank'&&(
                <div>
                  <div style={{marginBottom:14,padding:'10px 13px',background:'rgba(99,102,241,0.06)',border:'1px solid rgba(99,102,241,0.2)',borderRadius:8,fontSize:12,color:'var(--primary)'}}>ℹ️ Transfer the exact amount shown. Your plan activates within 24 hours of receipt.</div>
                  <div className="bank-details">
                    {[['Bank','Peoples Bank Sri Lanka'],['Account Name','Launchpad Technologies (Pvt) Ltd'],['Account Number','123-4567-8901-00'],['Branch','Colombo 03'],['Swift Code','PSBKLKLX'],['Amount',fmt(total)],['Reference',`LP-${(subdomain||'ORDER').toUpperCase()}`]].map(([k,v])=>(
                      <div key={k} className="bank-row">
                        <span className="bank-row-label">{k}</span>
                        <div style={{display:'flex',alignItems:'center',gap:8}}>
                          <span className="bank-row-value">{v}</span>
                          {['Account Number','Reference','Amount'].includes(k)&&<button className="copy-btn" onClick={()=>copyText(v,k)}>{copied===k?'Copied!':'Copy'}</button>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {method==='wallet'&&(
                <div>
                  <div className="wallet-grid">{WALLETS.map(w=><div key={w.id} className={`wallet-option ${wallet===w.id?'active':''}`} onClick={()=>setWallet(w.id)}><div className="wallet-emoji">{w.emoji}</div><div className="wallet-name">{w.name}</div><div className="wallet-desc">{w.desc}</div></div>)}</div>
                  <div className="phone-input-row">
                    <div className="phone-code">🇱🇰 +94</div>
                    <div className="input-wrap" style={{flex:1}}><span className="input-icon">📱</span><input type="text" placeholder="07X XXX XXXX" maxLength={10}/></div>
                  </div>
                  <div style={{marginTop:12,padding:'10px 13px',background:'rgba(99,102,241,0.06)',border:'1px solid rgba(99,102,241,0.15)',borderRadius:8,fontSize:12,color:'var(--text-muted)'}}>You'll receive a payment request on your {WMAPS[wallet]} app. Approve it to complete your payment.</div>
                </div>
              )}
            </div>
            ...
            {/* Billing cycle */}
            <div className="pay-card">
              <div className="pay-section-title"><span>2</span>Billing Cycle</div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
                {[{id:'monthly',label:'Monthly',sub:'Billed each month'},{id:'annual',label:'Annual',sub:`Save Rs. ${(pd.price*2).toLocaleString('en-LK')}/yr`}].map(b=>(
                  <div key={b.id} onClick={()=>setBilling(b.id)} style={{padding:'13px 15px',borderRadius:12,cursor:'pointer',transition:'all 0.2s',border:`1.5px solid ${billing===b.id?'var(--primary)':'var(--border)'}`,background:billing===b.id?'rgba(99,102,241,0.05)':'var(--surface2)',boxShadow:billing===b.id?'0 0 0 3px rgba(99,102,241,0.1)':'none'}}>
                    <div style={{fontWeight:700,fontSize:14,fontFamily:'Syne,sans-serif',color:'var(--text)',marginBottom:3,display:'flex',alignItems:'center',gap:6}}>
                      {b.label} {b.id==='annual'&&<span style={{background:'var(--success)',color:'white',borderRadius:4,padding:'1px 6px',fontSize:10,fontFamily:'DM Sans,sans-serif',fontWeight:700}}>SAVE 17%</span>}
                    </div>
          ...
          <div className="order-summary">
            <div className="pay-card" style={{marginBottom:0}}>
              <div className="pay-section-title" style={{marginBottom:14}}>Order Summary</div>
              <div className="summary-plan-badge">
                <div className="summary-plan-name">{pd.name}</div>
                <div className="summary-plan-sub">{subdomain||'yoursite'}.launchpad.io · {billing==='annual'?'Annual':'Monthly'}</div>
              </div>
              <div className="summary-line"><span className="lbl">{pd.name} ({billing})</span><span className="val">{fmt(price)}</span></div>
              {discount>0&&<div className="summary-line discount"><span className="lbl">Annual discount</span><span className="val">−{fmt(discount)}</span></div>}
              <div className="summary-line"><span className="lbl">VAT (8%)</span><span className="val">{fmt(tax)}</span></div>
              <div className="summary-divider"/>
              <div className="summary-line total"><span className="lbl">Total due</span><span className="val">{fmt(total)}</span></div>
              <button className="pay-now-btn" onClick={()=>{setLoading(true);setTimeout(()=>{setLoading(false);setPaid(true);},1800);}} disabled={loading||pd.price===0}>
                {loading?'⏳ Processing…':pd.price===0?'🚀 Launch Free':`🔒 Pay ${fmt(total)}`}
              </button>
              <div className="secure-badges">
                <span className="secure-badge">🔒 SSL Encrypted</span>
                <span className="secure-badge">✓ PCI Compliant</span>
                <span className="secure-badge">↩ 30-day refund</span>
              </div>
              <div style={{marginTop:14,padding:'11px',background:'var(--surface2)',borderRadius:10,border:'1px solid var(--border)',fontSize:12,color:'var(--text-muted)',textAlign:'center',lineHeight:1.5}}>
                By paying you agree to our <a href="#" style={{color:'var(--primary)',fontWeight:500}}>Terms of Service</a> and <a href="#" style={{color:'var(--primary)',fontWeight:500}}>Refund Policy</a>.
              </div>
            </div>
          </div>
        </div>
      ...
      {paid&&(
        <div className="toast">
          ...
          <div>
            <div style={{fontWeight:700,fontSize:14,color:'var(--text)'}}>Payment successful!</div>
            <div style={{fontSize:12,color:'var(--text-muted)'}}>Your site is live at {subdomain||'yoursite'}.launchpad.io</div>
          </div>
        </div>
      )}
    </div>
...
export default function App(){
  ...
  const [selSub,setSelSub]=useState('mysite');
  const goPayment=(plan,sub)=>{setSelPlan(plan);setSelSub(sub||'mysite');if(plan!=='free')setPage('payment');else{alert('Your free site is live! 🎉');}};
  return(
    <>
      <style>{styles}</style>
      <div className="app">
        <nav className="nav">
          ...
          <div className="nav-links">
            {page!=='builder'&&page!=='payment'&&<button className="nav-btn nav-btn-ghost" onClick={()=>setPage('builder')}>Try builder</button>}
            {page==='payment'&&<button className="nav-btn nav-btn-ghost" onClick={()=>setPage('builder')}>← Builder</button>}
        ...
        {page==='signin'&&<SignIn onSwitch={setPage} onSuccess={()=>setPage('builder')}/>}
        {page==='signup'&&<SignUp onSwitch={setPage} onSuccess={()=>setPage('builder')}/>}
        {page==='builder'&&<Builder onNext={goPayment}/>}
        {page==='payment'&&<Payment plan={selPlan} subdomain={selSub} onBack={()=>setPage('builder')}/>}
      </div>
    </>
  );
}
